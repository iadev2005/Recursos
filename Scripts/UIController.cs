...
public class UIController : MonoBehaviour
{
    ...
    private IEnumerator UpdateSpeed()
    {
        ...
        {
            carSpeed =  new Vector3(humanCar.carBody.linearVelocity.x, 0f, humanCar.carBody.linearVelocity.z).magnitude * 3.6f;
            if (carSpeed > 999f) { carSpeed = 999f; }
            if (gameType == Menu.GameTypes.quickRace2P)
            {
                carSpeedText[1].text = $"{(int)carSpeed} km/h";
                carSpeedB =  new Vector3(humanCarB.carBody.linearVelocity.x, 0f, humanCarB.carBody.linearVelocity.z).magnitude * 3.6f;
