...
public class Player : MonoBehaviour
{
    ...
    private IEnumerator IntervalUpdate()
    {
        while (true)
        {
            ...
            if ((controlType == ControlTypes.robot) && ((carBody.linearVelocity.sqrMagnitude < 4f) || (currentWaypoint == waypoints[waypoints.Count - 1])))
    ...
    {
        yield return delay;
        if ((carBody.linearVelocity.sqrMagnitude < 4f) || (currentWaypoint == waypoints[waypoints.Count - 1]))
