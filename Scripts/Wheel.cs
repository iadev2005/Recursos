...
public class Wheel : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        if (wheelCollider.GetGroundHit(out wheelHitInfo))
        {
            ...
            if (totalDrift >= minDriftSpeed)
            {
                ...
                driftPoint = wheelHitInfo.point + transform.forward + (carBody.linearVelocity * (Time.time - lastFixedUpdateTime));
    ...
    {
        localVelocity = transform.InverseTransformDirection(carBody.linearVelocity);
        totalDrift = Mathf.Abs(localVelocity.x);
        wheelAngularVelocity = wheelCollider.radius * ((2f * Mathf.PI * wheelCollider.rpm) / 60f);
        carForwardVelocity = Vector3.Dot(carBody.linearVelocity, transform.forward);
